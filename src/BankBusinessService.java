   ...
   public void withdraw(int accNo , double amount){
    ...
    public boolean accountNumValidation(int accNo) {
        try (Connection connection = getConnection.getconnection();
             PreparedStatement ps = connection.prepareStatement("SELECT accNo FROM bank.accountmaster WHERE accNo = ?");
        ){
            ps.setInt(1, accNo);
            try (ResultSet rs = ps.executeQuery()) {
                return rs.next(); // If next() returns true, the account number exists
                }
        } catch (SQLException e) {
            e.printStackTrace();
          }
